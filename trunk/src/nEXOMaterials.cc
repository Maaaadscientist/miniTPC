...
//  fLXeMat = new G4MaterialPropertiesTable(); 
  ...
  fLXeMat->AddConstProperty( "ELECTRICFIELD", 380.0*volt/cm ); //for missed nooks and crannies
...
*/
  //index 1, http://www.rit.edu/kgcoe/microsystems/lithography/thinfilms/cgi-bin/database.cgi?SiO2.csv (UV/visible data)
/*  G4double SiO2RefractiveIndex[nSiO2Entries] =
            { 1.455,    1.455,   1.455,   1.455,
              1.4635,   1.4668,  1.467,   1.47,
              1.475,    1.4787,  1.4815,  1.4866,
              1.496,    1.5077,  1.5239,  1.552,
              1.604,    1.7412,  2.2994,  1.800 };
*/
  //            2059.8, 1799.7, 1203.9, 643.8,
  //            480.1, 435.9, 434.0, 396.9,
  //            358.7, 340.4, 325.3, 303.5,
  //            274.9, 250.4, 226.5, 198.0,
  //            177.1, 149.4, 124.0, 98.4
  //index 2, the same link with index1, but using VUV data
  G4double SiO2RefractiveIndex[nSiO2Entries] =
            { 1.455,    1.455,   1.455,   1.455,
              1.4616,   1.4649,  1.465,   1.4687,
              1.4738,   1.4775,  1.4808,  1.4869,
              1.4971,   1.5104,  1.5302,  1.562,
              1.607,    1.7088,  2.2994,  1.800 };
  ...
  G4double SiO2Absorption[nSiO2Entries] =
            ...
            //  10*cm, 10*cm, 10*cm, 10*cm,
            //  10*cm, 10*cm, 10*cm, 10*cm };
            //{ 10e-5*um, 10e-5*um, 10e-5*um, 10e-5*um,
            //  10e-5*um, 10e-5*um, 10e-5*um, 10e-5*um,
            //  10e-5*um, 10e-5*um, 10e-5*um, 10e-5*um,
            //  10e-5*um, 10e-5*um, 10e-5*um, 10e-5*um,
            //  10e-5*um, 10e-5*um, 10e-5*um, 10e-5*um };
            { 10000*cm, 10000*cm, 10000*cm, 10000*cm,
              10000*cm, 10000*cm, 10000*cm, 10000*cm,
              10000*cm, 10000*cm, 10000*cm, 10000*cm,
              10000*cm, 10000*cm, 10000*cm, 10000*cm,
              10000*cm, 10000*cm, 10000*cm, 10000*cm };
  ...
  fSiO2Mat->AddProperty("RINDEX", SiO2Energy, SiO2RefractiveIndex, nSiO2Entries)->SetSpline(true);
  fSiO2Mat->AddProperty("ABSLENGTH", SiO2Energy, SiO2Absorption, nSiO2Entries)->SetSpline(true);
...
/*
  ...
  G4double SiAbsorption[nSiEntries] =
            { 1e-15*um, 1e-15*um, 1e-15*um, 1e-15*um,
              1e-15*um, 1e-15*um, 1e-15*um, 1e-15*um,
              1e-15*um, 1e-15*um, 1e-15*um, 1e-15*um,
              1e-15*um, 1e-15*um, 1e-15*um, 1e-15*um,
              1e-15*um, 1e-15*um, 1e-15*um, 1e-15*um,
  ...
  G4Material* Silicon =  fNistMan->FindOrBuildMaterial("G4_Si");
  fSiMat = new G4MaterialPropertiesTable();
  fSiMat->AddProperty("RINDEX", SiliconPhotonEnergy, siliconRealRefractiveIndex, nSiEntries)->SetSpline(true);
  fSiMat->AddProperty("ABSLENGTH", SiliconPhotonEnergy, SiAbsorption, nSiEntries)->SetSpline(true);
  Silicon->SetMaterialPropertiesTable( fSiMat );
*/
