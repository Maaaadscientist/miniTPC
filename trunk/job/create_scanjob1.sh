    ...
    do
      ...
      do
        ...
        do
          ...
          echo "/nEXO/det/bulgeRadius 1 cm" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          ...
          #echo "/rootfile/fileName /nfs/slac/g/exo_data4/users/caogf/nEXO/photonCollection/g410/lotusTPC_tubeSiPM_tubeFieldRing_ringSide40.85_AnodeCathodeSizeDesigned/run-fixedBugOfFieldRing-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          echo "/rootfile/fileName /nfs/slac/g/exo_data4/users/caogf/nEXO/photonCollection/g410/lotusTPC_tubeSiPM_tubeFieldRing_ringSide40.85_AnodeCathodeSizeDesigned/run-IncidentAngOnSiPM-GenX0-Y0-Z0-Dir010-RayScatt639AndTag-test-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          ...
          echo "/gps/direction 0 1 0 " >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          ...
          echo "/gps/ang/type planar" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          #echo "/gps/ang/type iso" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          echo "/gps/energy $energy eV" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          echo "/gps/particle $particle" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          echo "/run/beamOn 50000" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          ...
          bsub -W 7200 -R rhel60 -q xxl ./job-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.csh
        done
      done
    done
  done
