    ...
    do
      ...
      do
        ...
        do
          ...
          echo "/nEXO/det/fieldRingLength 10 mm" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          echo "/nEXO/det/fieldRingThick $B mm" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          echo "/nEXO/det/ringArrangeID 3" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          echo "/nEXO/det/ringSideDistance 30.85 mm" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          echo "/nEXO/det/radialRingDistance $C mm" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          ...
          echo "/rootfile/fileName /nfs/slac/g/exo_data4/users/caogf/nEXO/photonCollection/g410/tubeTPC_tubeSiPM_2wireFieldRing/run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
          ...
          echo "/run/beamOn 200000" >> run-RingRef$A-Cathode$A-Thick$B-Length$C-Distance$D-$n.in
