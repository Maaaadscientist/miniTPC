...
nEXOPrimaryGeneratorAction::~nEXOPrimaryGeneratorAction()
{
  ...
  if (generator == "nCaptureXe136"){
    delete particleGun2;
  }
  if (generator == "nCaptureCu"){
    delete particleGun2;
  }
...
void nEXOPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  if ( generator == "gps" || generator == "generalParticleSource" ){
    ...
    generate_bb0n(anEvent);
  }
  else if (generator == "bb2n") {
    particleGun2 = new G4ParticleGun();
    generate_bb2n(anEvent);
  }  
  else if (generator == "nCaptureXe136"){
    particleGun2 = new G4ParticleGun();
    generate_nCaptureXe136(anEvent, nCaptureXe_Method, XeComponent);
  }
...
void nEXOPrimaryGeneratorAction::set_generator(G4String value){
...
void nEXOPrimaryGeneratorAction::set_nCaptureXe_Method( G4String value)
{
  G4cout << "***************************************************" << G4endl;

  if (value != "InternalConversions" && value != "RandomGammas" && value != "ImbalancedCascade"){
    G4cout << "Error: must be InternalConversions, RandomGammas, or ImbalancedCascade" << G4endl;}  else {
    G4cout << "setting the method to " << value << G4endl;
    nCaptureXe_Method = value;
  }
  G4cout << "***************************************************" << G4endl;
}

void nEXOPrimaryGeneratorAction::set_Xe_Component( G4String value)
{
  G4cout << "***************************************************" << G4endl;

  if (value != "ActiveLXe" && value != "InactiveLXe"){
    G4cout << "Error: must be ActiveLXe or InactiveLXe" << G4endl;}  else {
...
void nEXOPrimaryGeneratorAction::set_Cu_isotope( G4int value)
{
  G4cout << "***************************************************" << G4endl;
...
void nEXOPrimaryGeneratorAction::set_Cu_Component( G4String value)
{
  G4cout << "***************************************************" << G4endl;

  if (value != "TPC" && value != "APDPlatter" && value != "FieldRing" && value != "InnerCryo" && value != "OuterCryo"){
    G4cout << "Error: must be the TPC, APDPlatter, Field Ring, InnerCryo, or OuterCryo" << G4endl;}  else {
    G4cout << "setting the component to " << value << G4endl;
    CuComponent = value;
  }
  G4cout << "***************************************************" << G4endl;
...
void nEXOPrimaryGeneratorAction::generate_bb0n( G4Event *anEvent )
{
    ...
    particleGun2->SetParticleDefinition(aBeta);
    ...
    G4double K, D;
    G4double D_spectral=0.; 
    G4double D_spectral_max;
    
    G4int iterations=100; //The max no. of iterations to satisfy 
                          //the Acceptance-Rejection condition


    K = T0; 
    D_spectral_max = D_bb0n_spectral_max(K); // Find the peak of the D spectrum first,

    for ( G4int n=0; n < iterations; n++){ // Acceptance-Rejection iteration loop
	  
      D = K*(2*G4UniformRand()-1.0);   // -K < D < +K
      
      D_spectral = D_spectral_max * G4UniformRand();
      
      // Test whether or not a randomly generated point is below the D spectrum
      if ( D_spectral <= D_bb0n_spectrum(K,D)){

        n = iterations; // Condition is met; stop the loop.
	T1 = 0.5*(K+D);
	T2 = 0.5*(K-D);
    ...
    T1 = T1*0.511*MeV; // scale by the electron mass
    T2 = T2*0.511*MeV;
  ...
  // Beta source vertex----------------------------------------
    ...
    //***** Remember that the axis of the TPC lies along the X coordinate!

    G4double LXeRadius = 65.*cm;
    G4double LXeHalfLength = 65.*cm;
  
  G4double phi0, r0, X0, Y0, Z0;
  G4ThreeVector decayVertex;


  bool XenonVolume = false;
  while ( XenonVolume == false ) {

    X0 = (2.0*G4UniformRand() - 1.0)*LXeHalfLength;  
    phi0 = 2*pi*G4UniformRand();
    r0 = LXeRadius*sqrt(G4UniformRand());
    Z0 = r0*cos(phi0);
    Y0 = r0*sin(phi0);
    ...
    //G4cout << "VolName = " << volume->GetName() << G4endl;
    
    if (volume->GetName()=="ActiveLXe"|| volume->GetName()=="InactiveLXe"){
      decayVertex = G4ThreeVector(X0, Y0, Z0);
      // set the particle gun position to source vertex
      particleGun2->SetParticlePosition(decayVertex);
      //Exit Loop
      XenonVolume = true;
    }
  }
  ...
  particleGun2->SetParticleEnergy(T1);
  ...
  particleGun2->SetParticleMomentumDirection(G4ThreeVector(ux1,uy1,uz1));
  ...
  G4double angular_distribution=0.;

  for ( G4int i=0; i < iterations; i++){
    theta12 = acos(2*(G4UniformRand()-0.5));
    angular_distribution =1.0 + beta1*beta2*(2*G4UniformRand()-1.0); 
    // 1 - beta1*beta2 < f(cos) < 1 + beta1*beta2
    ...
    if ( angular_distribution <= ( 1.0 - beta1*beta2*cos(theta12) ) ){
      ...
      i = iterations;   // Condition is met; stop the loop.
    }
  }
  ...
  // set the particle momentum direction
  particleGun2->SetParticleEnergy(T2);
  particleGun2->SetParticleMomentumDirection(G4ThreeVector(ux2,uy2,uz2));
  particleGun2->GeneratePrimaryVertex(anEvent);
...
// This function calculates the maximum value of the bb0n D spectrum.
  ...
  ff_factor =1;
    ...
    for (G4int n=0; n<N+1; n++) {
      ...
      phase_space[n] = p1*E1*fermi_function(Z,T1*ELECTRON_MASS)*p2*E2*fermi_function(Z,T2*ELECTRON_MASS);
...
G4double nEXOPrimaryGeneratorAction::D_bb0n_spectrum(G4double K, G4double D )
{
    ...
    return p1*E1*fermi_function(Z,T1*ELECTRON_MASS)*p2*E2*fermi_function(Z,T2*ELECTRON_MASS);
}
...
{
      particleGun2->SetParticleDefinition(aBeta);
  //    particleGun2->SetParticleDefinition(aGamma);
    ...
    G4double K, D, K_spectrum;
    G4double D_spectral=0.;
    G4double D_spectral_max;

    //G4cout << "K_spectral_max = " << K_spectral_max << G4endl;  

    // G4double K_spectral_max=0.0233;
    ...
    for ( G4int m=0; m < iterations; m++){

      K = T0*G4UniformRand();

      K_spectrum = K_spectral_max*G4UniformRand();

      if ( K_spectrum <= bb2n_sum_spectrum(K, D_spectral_max)) {
        // Acceptance-Rejection condition for the sum electron spectrum dN/dK

        for ( G4int n=0; n < iterations; n++){
          ...
          if ( D_spectral <= D_spectrum(K,D)){
            // Acceptance-Rejection condition for the single electron spectrum dN/To
        ...
        m = iterations; // Condition is met; stop the loop.
      }
    }
  ...
  particleGun2->SetParticleEnergy(T1);
  //G4cout << "T1 energy = " << T1/keV << "keV" << G4endl; // for troubleshooting
  ...
  // set the particle momentum direction
  particleGun2->SetParticleEnergy(T2);
  //G4cout<<"T2 energy = " << T2/keV << "keV" << G4endl; // for troubleshooting
...
G4double nEXOPrimaryGeneratorAction::bb2n_sum_spectrum( G4double K, G4double &D_spectral_max )
{
  ...
  // This function calculates the sum electron spectrum dN/dK for the bb2n decay.
    ...
    for (G4int n=0; n<N+1; n++) {
      ...
      phase_space[n] = p1*E1*fermi_function(Z,T1*ELECTRON_MASS)*p2*E2*fermi_function(Z,T2*ELECTRON_MASS)*pow(T0-K,5);
    }
    ...
    for (G4int i=0; i<N+1; i++){   //search for the peak value of phase space[i]
      if (phase_space[i] > D_spectral_max) D_spectral_max = phase_space[i];
    }


    // G4double normalization = 134958.7414*N;
    ...
    // N = # of bins. If N is changed, normalization should also be adjusted.
...
G4double nEXOPrimaryGeneratorAction::D_spectrum(G4double K, G4double D)
{
    ...
    return p1*E1*fermi_function(Z,T1*ELECTRON_MASS)*p2*E2*fermi_function(Z,T2*ELECTRON_MASS)*pow(T0-K,5);
}

G4double nEXOPrimaryGeneratorAction::SimpsonsRule(G4double x0, G4double xn, G4int N, G4double f[])
{
  //
  // Simpson's Rule:
...
void nEXOPrimaryGeneratorAction::set_norm( G4double q_value )
{
  ...
  // This function should only run once in the beginning.
   ...
   normalization = 1; //to make bb2n_sum_spectrum run the first time
    
   G4double bb2n_sum_max = 0;
   G4double bb2n_sum_spec[101]={0.};
   G4int N=100;
   for (G4int n=0; n<N+1; n++)
   {
   bb2n_sum_spec[n] = bb2n_sum_spectrum(n*T0/100, D_spectral_max);
   
   if (bb2n_sum_spec[n] > bb2n_sum_max) bb2n_sum_max = bb2n_sum_spec[n];
   }

   normalization = SimpsonsRule(0, T0, 100, bb2n_sum_spec);

   G4cout << "normalization " << normalization << G4endl;  

   K_spectral_max = bb2n_sum_max*1.01/normalization;

   G4cout << "K_spectral_max " << K_spectral_max << G4endl;

   norm_calculated = true; // so this only runs once
}


void nEXOPrimaryGeneratorAction::generate_nCaptureXe136( G4Event *anEvent , G4String nCaptureXe_Method, G4String XeComponent){
  //xG4cout << "in Generator" << G4endl;
  ...
  G4double  x1, y1, z1, phi1, theta1;
  
  G4double INACTIVE_XENON_R = 65.;
  G4double INACTIVE_XENON_Z = 65.;
  G4bool XenonVolume = false;
  while(XenonVolume == false){
    phi0 = 2*pi*G4UniformRand();
    r0 = INACTIVE_XENON_R*sqrt(G4UniformRand());
    x0 = INACTIVE_XENON_Z*(2*(G4UniformRand() - 0.5));
    y0 = r0*cos(phi0);
    z0 = r0*sin(phi0);
    ...
    if (volume->GetName()== XeComponent){
      //G4cout << XeComponent << " chosen" << G4endl;
      particleGun2->SetParticlePosition(G4ThreeVector(x0*cm, y0*cm, z0*cm));
      XenonVolume = true;
    }
  }
  if (nCaptureXe_Method == "InternalConversions" || nCaptureXe_Method == "RandomGammas"){
	...
	if (energy > 0.){
	  ...
	  particleGun2->SetParticleEnergy(energy*keV);
	  phi1 = 2*pi*G4UniformRand();
	  theta1 = pi*G4UniformRand();
	  x1 = sin(theta1)*cos(phi1);
	  y1 = sin(theta1)*sin(phi1);
	  z1 = cos(theta1);
	  
	  particleGun2->SetParticleMomentumDirection(G4ThreeVector(x1,y1,z1));
	  particleGun2->GeneratePrimaryVertex(anEvent);	  
	}
	if ( energy < 0.) {
	  if (nCaptureXe_Method == "InternalConversions"){
	  ...
	  else if (nCaptureXe_Method == "RandomGammas"){
	    ...
	    if (target > 610.) level += 500.;
	    {
	      ...
	      double vdx = sint*cos(phi);
	      double vdy = sint*sin(phi);
	      double vdz = cost;
	      particleGun2->SetParticleEnergy(energy*keV);
	      particleGun2->SetParticleMomentumDirection(G4ThreeVector(vdx,vdy,vdz));
	      particleGun2->GeneratePrimaryVertex(anEvent);	  
	    }
	    {
	      energy = level - target;
  ...
  if (nCaptureXe_Method == "ImbalancedCascade"){
    ...
    for (G4int j = 0; 0<= j && j < decay_num;){
      if (p < Decay[j].ratio){
          ...
          energy = Decay[j].gamma;
	if (energy > 0.){
	  particleGun2->SetParticleEnergy(energy*MeV);
	  phi1 = 2*pi*G4UniformRand();
	  theta1 = pi*G4UniformRand();
	  x1 = sin(theta1)*cos(phi1);
	  y1 = sin(theta1)*sin(phi1);
...
void nEXOPrimaryGeneratorAction::generate_nCaptureCu(G4Event *anEvent,G4int iso, G4String component)
{
  ...
  if(1){
    ...
    //**** Start choosing energies of gammas from cu65ng cascade ****//
     nCapture_CuInfo Decay[] = {
      ...
      {16,        1158.09,          0567.35,      0.21556122449},
