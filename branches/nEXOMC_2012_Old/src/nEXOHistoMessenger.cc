...
nEXOHistoMessenger::nEXOHistoMessenger(nEXOHisto* manager)
:histo (manager)
{
  ...
  ih->SetGuidance("histo number : from 0 to MaxEXOHisto");
...
nEXOHistoMessenger::~nEXOHistoMessenger()
{
  delete fileCmd;
  delete histoCmd;
  delete factoryCmd;
...
void nEXOHistoMessenger::SetNewValue(G4UIcommand* command,G4String newValues)
{
